...
const OrganizationMockContext = createContext<OrganizationStore | undefined>(
  undefined,
);

export function OrganizationMockProvider({
  children,
}: {
  children: ReactNode;
}) {
  ...
  function saveCentre(centre: Centre) {
    setCentres((current) =>
      current.some((item) => item.id === centre.id)
        ? current.map((item) => (item.id === centre.id ? centre : item))
        : [centre, ...current],
    );
  }
  ...
  return (
    <OrganizationMockContext.Provider
      value={{ centres, employees, saveCentre, saveEmployee, toggleEmployee }}
    >
      {children}
    </OrganizationMockContext.Provider>
  );
...
export function useOrganizationMock() {
  ...
  if (!context)
    throw new Error(
      "useOrganizationMock must be used within OrganizationMockProvider",
    );
  return context;
}
