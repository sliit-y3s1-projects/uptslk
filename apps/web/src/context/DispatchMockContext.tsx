...
import {
  initialDispatchApprovals,
  initialDispatchIncidents,
  initialTrips,
...
export function DispatchMockProvider({ children }: { children: ReactNode }) {
  ...
  const [incidents, setIncidents] = useState(() =>
    structuredClone(initialDispatchIncidents),
  );
  const [approvals, setApprovals] = useState(() =>
    structuredClone(initialDispatchApprovals),
  );
  function saveTrip(trip: Trip) {
    const conflict = trips.find(
      (item) =>
        item.id !== trip.id &&
        item.centreId === trip.centreId &&
        item.serviceDate === trip.serviceDate &&
        item.scheduledTime === trip.scheduledTime &&
        item.status !== "Cancelled" &&
        (item.bay === trip.bay ||
          item.vehicleId === trip.vehicleId ||
          item.driverId === trip.driverId),
    );
    if (conflict)
      throw new Error(
        `Conflict with ${conflict.id}: the bay, vehicle, or driver is already assigned at ${trip.scheduledTime}.`,
      );
    setTrips((current) =>
  ...
  function transitionTrip(id: string, status: TripStatus) {
    setTrips((current) =>
      current.map((item) =>
        item.id === id
          ? { ...item, status, updatedAt: "Updated just now" }
          : item,
      ),
    );
  }
  function cancelTrip(id: string, reason: string) {
    setTrips((current) =>
      current.map((item) =>
        item.id === id
          ? {
  ...
  function addIncident(incident: DispatchIncident) {
    setIncidents((current) => [incident, ...current]);
  }
  function updateIncident(id: string, status: DispatchIncident["status"]) {
    setIncidents((current) =>
      current.map((item) => (item.id === id ? { ...item, status } : item)),
    );
  }
  function addApproval(approval: DispatchApproval) {
...
export function useDispatchMock() {
  ...
  if (!context)
    throw new Error("useDispatchMock must be used within DispatchMockProvider");
  return context;
}
