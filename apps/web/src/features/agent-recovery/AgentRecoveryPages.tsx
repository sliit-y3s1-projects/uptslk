...
export function AgentRecoveryPage() {
  ...
  const start = useMutation({
    mutationFn: () =>
      agentRecoveryApi.start(incidentId, objective || undefined),
    onSuccess: (workflow) => {
  ...
  return (
    <main className="flex flex-1 flex-col gap-5 bg-muted/20 p-4">
      ...
      />
      <section className="rounded-xl border border-primary/30 bg-gradient-to-br from-primary/[0.06] via-card to-card p-5 shadow-sm sm:p-6">
        ...
        >
          ...
          >
            {start.isPending ? "Assessing..." : "Run recovery agents"}
          </Button>
        </form>
        {start.error && (
...
function RecoveryDetail({
  ...
  const { summary, trip, steps, approvals } = workflow;
        ...
        <aside className="space-y-4">
          ...
          <article className="rounded-xl border bg-card p-5">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              ...
              with inputs, outputs and timings.
            </p>
          </article>
        </aside>
      </section>
