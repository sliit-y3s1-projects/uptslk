...
type Seat = { seatNumber: string; isAvailable: boolean };
...
export function CheckoutPage() {
  ...
  const trips = useQuery({
    queryKey: ["booking-trips", routeId],
    enabled: Boolean(routeId),
    queryFn: () => apiClient<Trip[]>(`/api/v1/trips?routeId=${routeId}`),
  });
  const seats = useQuery({
    queryKey: ["booking-seats", tripId],
    enabled: Boolean(tripId),
    queryFn: () => apiClient<Seat[]>(`/api/v1/bookings/trips/${tripId}/seats`),
  });
  async function confirm() {
    if (!tripId || !seat) return;
    setError("");
    try {
      await apiClient("/api/v1/bookings/me", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ tripId, seatNumber: seat }),
      });
      setConfirmed(true);
    } catch (cause) {
      setError(
      ...
      <section className="mx-auto max-w-3xl rounded-2xl border border-slate-200 bg-white p-8">
        ...
        {confirmed ? (
          <div className="mt-8 rounded-xl border border-emerald-200 bg-emerald-50 p-5 text-emerald-800">
            ...
            <p className="mt-1 text-sm">Your seat {seat} has been reserved.</p>
          </div>
        ) : (
          <>
            <div className="mt-7 space-y-3">
              ...
              {trips.data?.map((trip) => (
                <button
                  ...
                  className={`w-full rounded-xl border p-4 text-left ${tripId === trip.id ? "border-primary bg-primary/5" : "border-slate-200"}`}
                >
            ...
            {tripId && (
              <div className="mt-7">
                <h2 className="font-semibold">Select a seat</h2>
                <div className="mt-3 grid grid-cols-5 gap-2">
                  {seats.data?.map((item) => (
                    <button
                      ...
                      onClick={() => setSeat(item.seatNumber)}
                      className={`rounded-lg border px-2 py-2 text-sm ${!item.isAvailable ? "cursor-not-allowed bg-slate-100 text-slate-400" : seat === item.seatNumber ? "border-primary bg-primary text-white" : "border-slate-200"}`}
                    >
                      {item.seatNumber}
                    </button>
                  ))}
