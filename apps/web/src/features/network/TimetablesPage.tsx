...
export function TimetablesPage() {
  ...
  const [scheduleDialog, setScheduleDialog] = useState<
    RouteSchedule | "new" | null
  >(null);
  const [generateSchedule, setGenerateSchedule] =
    useState<RouteSchedule | null>(null);
  const [generationResult, setGenerationResult] =
    useState<GenerateScheduleTripsResult | null>(null);
  const [generationDirection, setGenerationDirection] =
    useState<RouteDirectionContext | null>(null);
  const [tripSummary, setTripSummary] = useState<{
    direction: RouteDirectionContext;
    trips: TripListItem[];
  } | null>(null);
  ...
  const {
    ...
    isLoading,
    error,
  } = useSchedules(routeId || undefined);
  const { data: generatedTrips = [], isLoading: loadingGeneratedTrips } =
    useTrips({ routeId: routeId || undefined, date: reviewDate });
  ...
  const updateRoute = (value: string | null) => {
    if (value) {
      ...
      setSearchParams(
        nextDirection
          ? { route: value, direction: nextDirection, date: reviewDate }
          : { route: value, date: reviewDate },
      );
    }
  };
  ...
  const updateDirection = (value: string | null) => {
    if (value && routeId)
      setSearchParams({ route: routeId, direction: value, date: reviewDate });
  };
  ...
  const updateReviewDate = (value: string) => {
    setSearchParams(
      directionId || activeDirectionId
        ? {
            route: routeId,
            direction: directionId || activeDirectionId,
            date: value,
          }
        : { route: routeId, date: value },
    );
  };
  ...
  const saveSchedule = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (scheduleDialog && scheduleDialog !== "new") {
      updateSchedule.mutate(
        {
          scheduleId: scheduleDialog.id,
          data: { ...data, isActive: scheduleDialog.isActive },
        },
        { onSuccess: close },
      );
    } else {
  ...
  const generate = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    generateTrips.mutate(
      { scheduleId: generateSchedule.id, data: { serviceDate: date } },
      {
        onSuccess: (result) => {
  ...
  return (
    <main className="flex flex-1 flex-col gap-5 bg-muted/20 p-4">
      ...
      {!routeId ? (
        <EmptyState text="Choose a route to manage its recurring timetables." />
      ) : !activeDirectionId ? (
        <EmptyState text="Add a travel direction to this route before creating a timetable." />
      ) : error ? (
        <EmptyState text="Could not load timetable data. Please try again." />
      ) : isLoading ? (
              ...
              {schedules
                ...
                .map((schedule) => (
                  <ScheduleRow
                    ...
                    onGenerate={() => setGenerateSchedule(schedule)}
                    onDeactivate={() => deactivateSchedule.mutate(schedule.id)}
      ...
      >
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>
              {scheduleDialog === "new"
                ? "Add recurring timetable"
                : "Edit recurring timetable"}
            </DialogTitle>
            <DialogDescription>
              A timetable is the planned service pattern. It does not change
              previously generated trips.
            </DialogDescription>
          </DialogHeader>
          <ScheduleForm
            schedule={
      ...
      <Dialog
        open={generateSchedule !== null}
        onOpenChange={(open) => !open && setGenerateSchedule(null)}
      >
        <DialogContent className="sm:max-w-md">
          ...
          {direction && generateSchedule && (
            <div className="rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-950">
              ...
              <p className="mt-1 text-emerald-800">
                Generating from bay {generateSchedule.bayCode} for this
                direction only.
              </p>
            </div>
          )}
          <form className="grid gap-4" onSubmit={generate}>
            <label className="grid gap-1.5 text-sm font-medium">
              Service date
              <DatePicker name="serviceDate" defaultValue={today} required />
            </label>
            {generateTrips.error && (
              <p className="text-sm text-destructive">
                Unable to generate trips. Check vehicle, driver, bay
            ...
            <div className="flex justify-end gap-2">
              <Button
                type="button"
                variant="outline"
                onClick={() => setGenerateSchedule(null)}
              >
                Cancel
              </Button>
              <Button type="submit" disabled={generateTrips.isPending}>
                {generateTrips.isPending ? "Generating..." : "Generate trips"}
              </Button>
            </div>
          </form>
        </DialogContent>
      ...
      <Dialog
        open={tripSummary !== null}
        onOpenChange={(open) => !open && setTripSummary(null)}
      >
        <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-3xl">
          <DialogHeader>
            <DialogTitle>Generated trips</DialogTitle>
            <DialogDescription>
              {tripSummary && (
                <>
                  {tripSummary.direction.startCentreName}{" "}
                  <ArrowRight className="mx-1 inline size-4" />{" "}
                  {tripSummary.direction.endCentreName} · Service date{" "}
                  {reviewDate}
                </>
              )}
            </DialogDescription>
                ...
                <tbody className="divide-y">
                  {tripSummary.trips.map((trip) => (
                    <tr key={trip.id}>
                      ...
                      <td className="px-4 py-3">{trip.bay}</td>
                      <td className="px-4 py-3">
                        {trip.available} available / {trip.capacity}
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      <div className="flex min-w-0 items-start gap-3">
        ...
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <h3 className="font-medium">{schedule.operatingDays}</h3>
            <StatusBadge
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Every {schedule.headwayMinutes} min · Bay {schedule.bayCode}
          </p>
          <p className="mt-1 text-sm font-medium">
            {time(schedule.firstDeparture)} – {time(schedule.lastDeparture)}
          </p>
        </div>
      </div>
    ...
    <form className="grid gap-4" onSubmit={onSubmit}>
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <label className="grid gap-1.5 text-sm font-medium">
          ...
          <Select
            name="operatingDays"
            defaultValue={schedule?.operatingDays ?? "Everyday"}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
        ...
        <label className="grid gap-1.5 text-sm font-medium">
          ...
          <TimePicker
            name="firstDeparture"
            defaultValue={schedule ? time(schedule.firstDeparture) : "06:00"}
            required
          />
        </label>
        <label className="grid gap-1.5 text-sm font-medium">
          ...
          <TimePicker
            name="lastDeparture"
            defaultValue={schedule ? time(schedule.lastDeparture) : "18:30"}
            required
          />
        </label>
        <label className="grid gap-1.5 text-sm font-medium sm:col-span-2">
          ...
          <Select
            name="headwayMinutes"
            defaultValue={String(schedule?.headwayMinutes ?? 30)}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
...
function EmptyState({ text }: { text: string }) {
  return (
    <div className="rounded-xl border border-dashed border-slate-300 bg-card px-6 py-14 text-center text-sm text-muted-foreground">
      {text}
    </div>
  );
}
...
function GenerationSummary({
  ...
  result: GenerateScheduleTripsResult;
  direction: RouteDirectionContext | null;
}) {
  const date = new Date(`${result.serviceDate}T00:00:00`).toLocaleDateString(
    undefined,
  ...
  return (
    <section className="rounded-xl border border-slate-300 bg-card p-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="font-semibold">Trip generation summary</h2>
          <p className="text-sm text-muted-foreground">
            {date} · Generated departures appear on the Dispatch board.
          </p>
        </div>
        <Button
          variant="outline"
          render={
            <Link to={`/operations/dispatch?date=${result.serviceDate}`} />
          }
        >
      ...
      {direction && (
        <div className="mt-4 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-950">
          ...
          <ArrowRight className="mx-1 inline size-4" />{" "}
          {direction.endCentreName}
          <span className="text-emerald-800"> · Bay {direction.bayCode}</span>
        </div>
      )}
      <div className="mt-4 grid gap-3 sm:grid-cols-3">
        <SummaryStat
          label="Added to dispatch"
          value={result.created}
          detail="Ready for the dispatch board"
          tone="green"
        />
        <SummaryStat
        ...
        <SummaryStat
          ...
          detail="Could not be assigned automatically"
          tone="amber"
        />
      </div>
      {result.createdDepartures.length > 0 && (
        <div className="mt-4 border-t border-slate-200 pt-4">
          <p className="text-sm font-medium">Departures added</p>
          <div className="mt-2 flex flex-wrap gap-2">
            {result.createdDepartures.map((departure) => (
              <span
                key={departure}
                className="rounded-md border border-emerald-200 bg-emerald-50 px-2.5 py-1 text-sm text-emerald-800"
              >
                {departure}
              </span>
            ))}
      ...
      {result.skippedDepartures.length > 0 && (
        <div className="mt-4 border-t border-slate-200 pt-4">
          <p className="text-sm font-medium">Departures to review</p>
          <p className="mt-1 text-sm text-muted-foreground">
            Use the Dispatch board to assign a different bus, driver, or bay for
            these times.
          </p>
          <div className="mt-2 divide-y rounded-lg border border-slate-300">
            {result.skippedDepartures.map((departure) => (
              <div
                key={`${departure.time}-${departure.reason}`}
...
function SummaryStat({
  ...
  return (
    <div className={`rounded-lg border p-3 ${colours}`}>
