...
export function RouteDetailPage({
  ...
  const [editingDirection, setEditingDirection] = useState<RouteDirection | null>(null);
  ...
  return (
    <main className="flex flex-1 flex-col gap-4 bg-muted/20 p-4">
      ...
      <Dialog open={editingDirection !== null} onOpenChange={(open) => !open && setEditingDirection(null)}><DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-xl"><DialogHeader><DialogTitle>Edit route direction</DialogTitle><DialogDescription>Update the terminal pair and travel time. Existing timetables remain associated with this direction.</DialogDescription></DialogHeader>{editingDirection && <DirectionForm direction={editingDirection} centres={centres} pending={updateDirection.isPending} onCancel={() => setEditingDirection(null)} onSubmit={(data) => updateDirection.mutate({ directionId: editingDirection.id, data: { ...data, isActive: editingDirection.isActive } }, { onSuccess: () => setEditingDirection(null) })} />}</DialogContent></Dialog>
    </main>
  );
}
