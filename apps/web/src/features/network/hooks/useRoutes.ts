...
import type { CreateRouteRequest, UpdateRouteRequest, CreateRouteScheduleRequest, UpdateRouteScheduleRequest } from '../types';
...
export function useUpdateRoute(routeId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: UpdateRouteRequest) => routesApi.updateRoute(routeId, data),
    onSuccess: () => {
...
export function useCreateSchedule(routeId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: CreateRouteScheduleRequest) => routesApi.createSchedule(routeId, data),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['routes', routeId] })
  });
}
...
export function useUpdateSchedule(routeId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ scheduleId, data }: { scheduleId: string; data: UpdateRouteScheduleRequest }) => routesApi.updateSchedule(scheduleId, data),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['routes', routeId] })
  });
}
...
export function useDeactivateSchedule(routeId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (scheduleId: string) => routesApi.deactivateSchedule(scheduleId),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['routes', routeId] })
  });
}
