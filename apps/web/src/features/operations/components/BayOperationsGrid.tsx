        ...
        {bays.map((bay) => (
          ...
          >
            <div className="flex items-center justify-between">
              <span className="rounded-md border border-current/15 bg-white/50 px-2 py-1 text-sm font-bold">{bay.bay}</span>
            ...
            {bay.vehicle ? (
              <>
                ...
                <p className="mt-2 truncate text-base font-semibold">{bay.route} · {bay.destination}</p>
                <div className="mt-4 flex items-center justify-between border-t border-current/10 pt-3 text-xs text-muted-foreground">
                  ...
                  {(bay.queue ?? 0) > 0 && <span className="flex items-center gap-1"><UsersRound className="size-3" /> {bay.queue} waiting</span>}
                </div>
              </>
            ) : (
