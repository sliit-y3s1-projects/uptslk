...
export function IncidentsPage() {
  ...
  const {
    ...
    isLoading,
    error,
  } = useIncidents(user?.centreId);
  const updateMutation = useUpdateIncident();
  if (isLoading) return <main className="p-5">Loading incidents...</main>;
  if (error)
    return <main className="p-5 text-red-600">Failed to load incidents.</main>;
  return (
    <main className="flex flex-1 flex-col gap-4 bg-muted/20 p-4">
      <PageHeading
        title="Incidents"
        description="Triage operational and safety issues connected to centre trips."
        action={
          <Button render={<Link to="/operations/incidents/new" />}>
          ...
          <article key={status} className="rounded-lg border bg-card">
            <header className="flex items-center justify-between border-b px-4 py-3">
              <h2 className="font-semibold">
                {status === "InProgress" ? "Investigating" : status}
              </h2>
                ...
                .map((incident) => (
                  <div key={incident.id} className="rounded-md border p-3">
                    ...
                    <p className="mt-3 text-xs text-muted-foreground">
                      {incident.assignedTo ?? incident.reportedByName} ·{" "}
