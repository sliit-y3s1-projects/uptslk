...
export function RequestQueue({
  ...
  const updateStatus = (
    request: typeof query.data extends (infer T)[] | undefined ? T : never,
  ...
  return (
    <main className="flex flex-1 flex-col gap-4 bg-muted/20 p-4">
      ...
      <Feedback
        error={query.error || mutations.create.error || mutations.update.error}
        success={notice}
      />
      <Panel title="Create request">
        <div className="grid gap-3 md:grid-cols-[2fr_1fr]">
          <Field
            ...
            placeholder="What does the passenger need?"
          />
          <SelectField
            label="Priority"
            value={priority}
          ...
          >
            {(
              ["Low", "Medium", "High", "Urgent"] as SupportRequestPriority[]
            ).map((item) => (
        ...
        <Button
          onClick={create}
          disabled={
            !subject.trim() || !details.trim() || mutations.create.isPending
          }
        >
          {mutations.create.isPending ? "Creating..." : "Create request"}
        </Button>
      </Panel>
          ...
          >
            {query.data.map((request) => (
              <tr key={request.id}>
                ...
                <td>{request.passenger ?? "Unlinked"}</td>
                ...
                <td className="space-x-2">
                  {request.status === "Open" && (
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => updateStatus(request, "InProgress")}
                    >
                      Assign
                    </Button>
                  )}
