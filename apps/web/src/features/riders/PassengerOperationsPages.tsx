...
export function PassengerFlowPage() {
  ...
  const bookings = useMemo(() => {
    ...
    return [...(manifest.data?.bookings ?? [])]
      ...
      .filter((booking) => !term || [booking.passenger, booking.phoneNumber, booking.qrCode].some((value) => value?.toLowerCase().includes(term)));
        ...
        {tripId && <>
          ...
          {manifest.data && !manifest.error && <>
            <section className="grid gap-3 sm:grid-cols-3">
              ...
              <FlowStat label="Capacity" value={manifest.data.trip.capacity} detail={`${manifest.data.passengerCount} spaces currently booked`} />
            ...
            <Panel title="Passenger manifest">
              <div className="flex flex-col gap-4 border-b pb-4 lg:flex-row lg:items-end lg:justify-between">
                ...
                <Button variant="outline" disabled={manifest.isFetching} onClick={() => { void manifest.refetch(); }}>Refresh manifest</Button>
              </div>
              <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_220px_auto]">
                <Field label="Search passenger, phone or QR reference" value={search} onChange={(event) => { setSearch(event.target.value); setPage(0); }} placeholder="Search manifest" />
              ...
              {bookings.length ? <DataTable headings={["Passenger", "Count", "Contact", "Category", "Status", "QR reference", "Action"]}>{visibleBookings.map((booking) => <tr key={booking.id}><td className="font-medium">{booking.passenger}</td><td className="font-semibold">{booking.passengerCount}</td><td>{booking.phoneNumber}</td><td>{booking.category}</td><td><StatusPill status={booking.status} /></td><td className="max-w-36 truncate font-mono text-xs" title={booking.qrCode}>{booking.qrCode}</td><td><Link to={`/fares/bookings?bookingId=${booking.id}`}><Button size="sm" variant="outline">View ticket</Button></Link></td></tr>)}</DataTable> : <p className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">No passengers match these filters.</p>}
